...
{
	bool is_zero = false;
	if (MABS(a[0]) < MFLT_EPSILON && MABS(a[1]) < MFLT_EPSILON) {
		is_zero = true;
	}
	return is_zero;
}
...
bool vec2_is_near_zero(mfloat_t *a, mfloat_t epsilon)
{
	...
	if (MABS(a[0]) < epsilon && MABS(a[1]) < epsilon) {
		is_near_zero = true;
	}
	return is_near_zero;
}
...
{
	bool is_equal = false;
	if (MABS(a[0] - b[0]) < epsilon && MABS(a[1] - b[1]) < MFLT_EPSILON) {
		is_equal = true;
	}
	return is_equal;
}
...
{
	bool is_zero = false;
	if (MABS(a[0]) < MFLT_EPSILON && MABS(a[1]) < MFLT_EPSILON && MABS(a[2]) < MFLT_EPSILON) {
		is_zero = true;
	}
	return is_zero;
}
...
{
	bool is_near_zero = false;
	if (MABS(a[0]) < epsilon && MABS(a[1]) < epsilon && MABS(a[2]) < epsilon) {
		is_near_zero = true;
	}
	return is_near_zero;
}
...
{
	bool is_equal = false;
	if (MABS(a[0] - b[0]) < epsilon && MABS(a[1] - b[1]) < MFLT_EPSILON && MABS(a[2] - b[2]) < MFLT_EPSILON) {
		is_equal = true;
	}
	return is_equal;
}
...
mfloat_t *vec3_multiply_mat3(mfloat_t *result, mfloat_t *a, mfloat_t *b)
{
	...
	mfloat_t y = a[1];
	mfloat_t z = a[2];
	result[0] = (b[0] * x) + (b[1] * y) + (b[2] * z);
	result[1] = (b[3] * x) + (b[4] * y) + (b[5] * z);
	result[2] = (b[6] * x) + (b[7] * y) + (b[8] * z);
	return result;
}
...
{
	bool is_zero = false;
	if (MABS(a[0]) < MFLT_EPSILON && MABS(a[1]) < MFLT_EPSILON && MABS(a[2]) < MFLT_EPSILON && MABS(a[3]) < MFLT_EPSILON) {
		is_zero = true;
	}
	return is_zero;
}
...
{
	bool is_near_zero = false;
	if (MABS(a[0]) < epsilon && MABS(a[1]) < epsilon && MABS(a[2]) < epsilon && MABS(a[3]) < epsilon) {
		is_near_zero = true;
	}
	return is_near_zero;
}
...
{
	bool is_equal = false;
	if (MABS(a[0] - b[0]) < epsilon && MABS(a[1] - b[1]) < MFLT_EPSILON && MABS(a[2] - b[2]) < MFLT_EPSILON && MABS(a[3] - b[3]) < MFLT_EPSILON) {
		is_equal = true;
	}
	return is_equal;
}
...
mfloat_t *vec4_multiply_mat4(mfloat_t *result, mfloat_t *a, mfloat_t *b)
{
	...
	mfloat_t z = a[2];
	mfloat_t w = a[3];
	result[0] = (b[0] * x) + (b[1] * y) + (b[2] * z) + (b[3] * w);
	result[1] = (b[4] * x) + (b[5] * y) + (b[6] * z) + (b[7] * w);
	result[2] = (b[8] * x) + (b[9] * y) + (b[10] * z) + (b[11] * w);
	result[3] = (b[12] * x) + (b[13] * y) + (b[14] * z) + (b[15] * w);
	return result;
}
...
mfloat_t *quat_divide(mfloat_t *result, mfloat_t *a, mfloat_t *b)
{
	...
	mfloat_t z = a[2];
	mfloat_t w = a[3];
	mfloat_t length_squared = MFLOAT_C(1.0) / (b[0] * b[0] + b[1] * b[1] + b[2] * b[2] + b[3] * b[3]);
	mfloat_t normalized_x = -b[0] * length_squared;
	mfloat_t normalized_y = -b[1] * length_squared;
	mfloat_t normalized_z = -b[2] * length_squared;
...
mfloat_t *quat_from_vec3(mfloat_t *result, mfloat_t *a, mfloat_t *b)
{
	...
	quat(result, cross[0], cross[1], cross[2], dot + MSQRT(a_length_sq * b_length_sq));
	quat_normalize(result, result);
	return result;
}
...
mfloat_t *quat_from_mat4(mfloat_t *result, mfloat_t *m)
{
	...
	if (scale > MFLOAT_C(0.0)) {
		...
		result[1] = (m[1] + m[4]) * half;
		result[2] = (m[2] + m[8]) * half;
		result[3] = (m[6] - m[9]) * half;
	} else if (m[5] > m[10]) {
		sr = MSQRT(MFLOAT_C(1.0) + m[5] - m[0] - m[10]);
		half = MFLOAT_C(0.5) / sr;
		result[0] = (m[4] + m[1]) * half;
		result[1] = MFLOAT_C(0.5) * sr;
		result[2] = (m[9] + m[6]) * half;
		result[3] = (m[8] - m[2]) * half;
	} else {
		sr = MSQRT(MFLOAT_C(1.0) + m[10] - m[0] - m[5]);
		half = MFLOAT_C(0.5) / sr;
		result[0] = (m[8] + m[2]) * half;
		result[1] = (m[9] + m[6]) * half;
		result[2] = MFLOAT_C(0.5) * sr;
		result[3] = (m[1] - m[4]) * half;
	}
	return result;
}
...
mfloat_t *mat4_inverse(mfloat_t *result, mfloat_t *m)
{
	...
	inverse[15] = m[0] * m[5] * m[10] -
		...
		m[2] * m[4] * m[9] -
		m[2] * m[8] * m[5];
	det = m[0] * inverse[0] + m[4] * inverse[1] + m[8] * inverse[2] + m[12] * inverse[3];
...
mfloat_t *mat4_look_at_with_up(mfloat_t *result, mfloat_t *position, mfloat_t *target, mfloat_t *up_axis)
{
	...
	result[1] = side_axis[1];
	result[2] = side_axis[2];
	result[3] = -vec3_dot(side_axis, position);
	result[4] = up_axis[0];
	result[5] = up_axis[1];
	result[6] = up_axis[2];
	result[7] = -vec3_dot(up_axis, position);
	result[8] = -forward_axis[0];
	result[9] = -forward_axis[1];
	result[10] = -forward_axis[2];
	result[11] = vec3_dot(forward_axis, position);
	result[12] = MFLOAT_C(0.0);
	result[13] = MFLOAT_C(0.0);
	result[14] = MFLOAT_C(0.0);
	result[15] = MFLOAT_C(1.0);
...
mfloat_t *mat4_look_at(mfloat_t *result, mfloat_t *position, mfloat_t *target)
{
	...
	vec3_subtract(forward_axis, target, position);
	vec3_normalize(forward_axis, forward_axis);
	if (nearly_equal(forward_axis[0], MFLOAT_C(0.0), MFLT_EPSILON) && nearly_equal(forward_axis[2], MFLOAT_C(0.0), MFLT_EPSILON)) {
		if (forward_axis[1] > MFLOAT_C(0.0)) {
